...
import FuelTypeForm from './FuelTypeForm.vue'

const props = defineProps({
  modelValue: {
    // v-model للتحكم في الظهور
    type: Boolean,
    default: false,
  },
  item: {
    // الكائن المراد تعديله (أو null للإضافة)
    type: Object,
    default: null,
  },
  isSaving: {
...
const title = computed(() =>
  props.item && props.item.id ? 'تعديل نوع الوقود' : 'إضافة نوع وقود جديد',
)

// مزامنة حالة الفتح والإغلاق
watch(
  () => props.modelValue,
  (val) => {
    isOpen.value = val
...
// مزامنة البيانات عند تغيير العنصر المحدد
watch(
  () => props.item,
  (val) => {
    itemData.value = val
